...
async function extract(page, id, type, season, episode) {
    let embedUrl = `https://vidfast.vc/embed/${type}/${id}`;
    ...
    const routeHandler = async (route) => {
        ...
        if (url.includes('.m3u8') || url.includes('/master.json') || url.includes('playlist.m3u8')) {
    ...
    try {
        ...
        const player = await page.$('#player, video, .vjs-big-play-button');
...
function isAd(url) {
    return /adsystem|popads|analytics|doubleclick|googlesyndication/.test(url);
}
